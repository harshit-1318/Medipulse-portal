...
import { ArrowUpDown, ArrowUp, ArrowDown, Activity, User, Hash, Layers, Clock } from "lucide-react";
...
export function ActivityTableHeader({ table }: Props) {
    return (
            ...
            {table.getHeaderGroups().map((hg) => (
                <tr key={hg.id}>
                    {hg.headers.map((h) => {
                        ...
                        const Icon = ICON_MAP[h.id] || ICON_MAP[(h.column.columnDef as any).accessorKey];

                        return (
                            <th
                                key={h.id}
                                className="px-5 py-4 font-bold whitespace-nowrap border-b border-slate-100 text-center group/th"
