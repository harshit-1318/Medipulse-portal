...
const DEFAULT_FILTERS: OrderFilters = {
    ...
    productCategory: "acid-reflux",
};

export function useAcidRefluxOrdersData() {
    const storageKey = getDashboardStorageKey("acid-reflux");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("acid-reflux-orders");
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("acid-reflux-orders");
        });
