...
const DEFAULT_FILTERS: OrderFilters = {
    ...
    productCategory: "bacterial-vaginosis",
};

export function useBacterialVaginosisOrdersData() {
    const storageKey = getDashboardStorageKey("bacterial-vaginosis");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("bacterial-vaginosis-orders");
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("bacterial-vaginosis-orders");
        });
