...
const DEFAULT_FILTERS: OrderFilters = {
    ...
    productCategory: "eczema-dermatitis",
};

export function useEczemaDermatitisOrdersData() {
    const storageKey = getDashboardStorageKey("eczema-dermatitis");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("eczema-dermatitis-orders");
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("eczema-dermatitis-orders");
        });
