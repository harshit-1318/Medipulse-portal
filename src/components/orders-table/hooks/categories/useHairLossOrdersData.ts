...
const DEFAULT_FILTERS: OrderFilters = {
    ...
    category: "mens-hair-loss",
    productCategory: "mens-hair-loss",
};

export function useHairLossOrdersData() {
    const storageKey = getDashboardStorageKey("mens-hair-loss");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("mens-hair-loss-orders");
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("mens-hair-loss-orders");
        });
