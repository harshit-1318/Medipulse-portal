...
const DEFAULT_FILTERS: OrderFilters = {
    ...
    category: "jet-lag-treatment",
    productCategory: "jet-lag-treatment",
};

export function useJetLagOrdersData() {
    const storageKey = getDashboardStorageKey("jet-lag-treatment");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("jet-lag-treatment-orders");
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("jet-lag-treatment-orders");
        });
