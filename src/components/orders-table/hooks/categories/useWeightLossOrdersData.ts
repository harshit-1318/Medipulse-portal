...
export function useWeightLossOrdersData() {
    const storageKey = getDashboardStorageKey("weight-loss");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("weight-loss-orders");
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("weight-loss-orders");
        });
