...
export function useUploadedDocsData() {
    const storageKey = getDashboardStorageKey("uploaded-docs");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("uploaded-docs-orders");

        getOrders(page, { ...filters, documents: "Uploaded" }).then((res) => {
            if (!ignore) {
            ...
            if (!ignore) stopLoader("uploaded-docs-orders");
        });
