...
export function useCancelledOrdersData() {
    const storageKey = getDashboardStorageKey("cancelled");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("cancelled-orders");

        getOrders(page, { ...filters, fulfillmentStatus: "cancelled", status: "" }).then((res) => {
            if (!ignore) {
            ...
            if (!ignore) stopLoader("cancelled-orders");
        });
