...
export function useFulfilledOrdersData() {
    const storageKey = getDashboardStorageKey("fulfilled");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("fulfilled-orders");

        getOrders(page, { ...filters, fulfillmentStatus: "fulfilled", status: "" }).then((res) => {
            if (!ignore) {
            ...
            if (!ignore) stopLoader("fulfilled-orders");
        });
