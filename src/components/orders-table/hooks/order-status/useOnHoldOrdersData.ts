...
export function useOnHoldOrdersData() {
    const storageKey = getDashboardStorageKey("on-hold");
    ...
    useEffect(() => {
        let ignore = false;
        setLoading(true);
        startLoader("on-hold-orders");

        getOrders(page, { ...filters, fulfillmentStatus: "on_hold", status: "" }).then((res) => {
            if (!ignore) {
            ...
            if (!ignore) stopLoader("on-hold-orders");
        });
