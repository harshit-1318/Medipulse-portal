...
export function useInjectableOrdersData() {
    const storageKey = getDashboardStorageKey("injectable");
    ...
    useEffect(() => {
        ...
        getOrders(page, apiFilters).then((res) => {
            ...
            if (!ignore) stopLoader("injectable-orders");
        });
    ...
    return {
        orders, page, setPage, total, loading, filters, setFilters, filtersEnabled, setFiltersEnabled
    };
}
